...
抖音现已完成自己的账号两场实际开播（9分7秒、9分52秒，第二场用户发起），并确认结束。公开房间采集不能替代自己的直播音画验收；详见本轮实播报告。
...
现有机器已安装，无需重复执行。改文案/规则使用工作台“模板/规则”页面；任务保存原版本，修改只影响新任务。展示页的奖励档位直接读取启用规则；静态PNG与HTML须另行更新，不自动随着运营编辑变化。
...
原生窗口菜单“导出活动图片”（Command+E）生成无鼠标PNG，本机导出2160×3840。中泰分别打开对应房间画面再导出，保存到 /Volumes/M2USB/Projects/ai-live-studio/public/live-assets 。

Dycast修订已补齐“先转发后连接房间”的握手；固定版本源码、补丁和回归在 /Volumes/M2USB/Projects/ai-live-studio/integrations/dycast 。启动前独立检查全局暂停状态：连接房间不等于恢复动作。第二场平台回放已确认收到欢迎音画；修订后的应用本地编译/启动与回调测试通过，未另外开启第三场实播。
