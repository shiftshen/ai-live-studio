...
当前交付是可运行的本地工作台，**尚未完成全部现场验收**。52项自动测试与24项Dycast补丁断言通过。两平台已收到真实评论、进场、点赞和关注；飞鹅相机纸面验证发现并修复字体通道问题。双平台30分钟真实采集已通过；TikTok真实礼物已触发打印回执，但发现并修复连送字段缺陷，已部署、尚待真实复验；真实礼物可靠闭环、动态头像出纸、OBS可听播放仍须按验收记录完成。两小时服务存活通过，但抖音末段下播使双平台全程连续项未通过。
...
7. **OBS 展示**：使用房间展示 URL 创建浏览器源，按需启用播放。语音在播放器 `ended` 后 ACK；无播放器时 `ready` 任务会保留。展示 token 只授权该房间。
